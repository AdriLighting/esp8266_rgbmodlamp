...
uint8_t LAMP::effectsLoop_hue(){
  ...
  for (int i = 0; i < _outPutCount; ++i) {
    if (_DeviceGet->get_outputArray(i)->get_onoff() && _DeviceGet->get_outputArray(i)->get_select() ) {
      if (_DeviceGet->get_outputArray(i)->get_effectType() == OET_HUETIMER) cnt++;
      if (_DeviceGet->get_outputArray(i)->get_effectType() == OET_HTPG) {cnt++; }
    }
  }
  return cnt;
}
uint8_t LAMP::effectsLoop_pal(){
  ...
  for (int i = 0; i < _outPutCount; ++i) {
    if (_DeviceGet->get_outputArray(i)->get_onoff() && _DeviceGet->get_outputArray(i)->get_select() ) {
      if (_DeviceGet->get_outputArray(i)->get_effectType() == OET_PAL) cnt++;
    }
  }
  return cnt;
}
uint8_t LAMP::effectsLoop_palGrad(){
  uint8_t cnt=0;
  for (int i = 0; i < _outPutCount; ++i) {
    if (_DeviceGet->get_outputArray(i)->get_onoff() && _DeviceGet->get_outputArray(i)->get_select() ) {
      if (_DeviceGet->get_outputArray(i)->get_effectType() == OET_PALGRAD) cnt++;
      if (_DeviceGet->get_outputArray(i)->get_effectType() == OET_HTPG) cnt++;
    }
  }
  return cnt;
...
void LAMP::effectsLoop_palGrad_reset(uint8_t p){
  _palGraddPos = p;
  _timer_effectsLoopPalGrad = millis();
}
...
void LAMP::effectsLoop(){
  ...
  if (isPalgradLoopOn()){
    if ((millis()-_timer_effectsLoopPalGrad)>_effectsLoop_timerPalGradMax){
        _palGraddPos++;
        _timer_effectsLoopPalGrad = millis();
    }     
  } 
